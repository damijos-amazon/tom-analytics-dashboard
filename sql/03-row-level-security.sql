-- TOM Analytics Dashboard - Row Level Security Policies
...
-- Only super admin can insert new users (typically done via auth trigger)
...
-- Admins can update user status (block/unblock)
...
WITH CHECK (
    -- Cannot modify super admin accounts unless you are super admin
...
-- All authenticated active users can insert/update employee records
CREATE POLICY "Authenticated users can modify employee records"
...
-- Only super admins can read SSO configuration
CREATE POLICY "Super admins can read SSO config"
...
-- Only super admins can modify SSO configuration
CREATE POLICY "Super admins can modify SSO config"
